...
public class VoiceOverTranslationPatch {
    ...
    public static void setVideoState(Enum<?> youTubeVideoState) {
        ...
        final String name = youTubeVideoState.name();
        mainHandler.post(() -> {
    ...
    public static void onPlaybackSpeedChanged(float speed) {
        mainHandler.post(() -> {
    ...
    public static void newVideoStarted(String videoId, boolean isShort) {
        ...
        pendingIsLive = isShort; // Shorts are treated as unavailable for translation
    }
    ...
    public static void toggleTranslation() {
        ...
        if (videoLength <= 0) {
            videoLength = pendingVideoLength;
        }
    ...
    public static void refreshOriginalAudioVolume() {
        ...
        float currentVolume = getPlayerVolume();
    ...
    public static void restartTranslationIfActive() {
        ...
        if (videoLength > 4 * 60 * 60 * 1000L) return;
        ...
        stopAudioPlayback();
        double durationSeconds = videoLength / 1000.0;
        final String videoIdFinal = videoId;
        Utils.runOnBackgroundThread(() -> requestTranslation(
                videoIdFinal, pendingVideoTitle,
                sourceLang, targetLang,
                durationSeconds
        ));
    }
    ...
    private static void applyPlaybackSpeedToPlayer(MediaPlayer mp) {
        if (mp == null) return;
        float speed = getPlaybackSpeedFromPlayer();
        if (speed <= 0f) {
    ...
    // ==========================================
    // Reflective Helpers for Volume/Speed targets
    // ==========================================

    private static Object getPlayerControllerFromVideoInformation() {
        try {
            java.lang.reflect.Field f = VideoInformation.class.getDeclaredField("playerControllerRef");
            f.setAccessible(true);
            java.lang.ref.WeakReference<?> ref = (java.lang.ref.WeakReference<?>) f.get(null);
    ...
    private static Object getVolumeTarget() {
        Object obj = getPlayerControllerFromVideoInformation();
        if (obj != null) {
    ...
    public static void setPlayerVolume(float volume) {
        Object target = getVolumeTarget();
        if (target == null) {
            Logger.printInfo(() -> "setPlayerVolume: no target found");
            return;
        }
        try {
